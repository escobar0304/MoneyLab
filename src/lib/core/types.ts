...
/**
 * A later verdict on whether a purchase was worth it — asked once the dust
 * has settled, not at the moment of buying. Same shape of idea as
 * `EntryClearedEvent`: not part of what happened, an assertion made about it
 * afterwards, so it lives beside the expense rather than mutating it.
 */
export interface WorthItEvent extends LedgerEventBase {
  type: 'worth_it';
  entryId: ID;
  worthIt: boolean;
}
