...
/**
 * The latest "worth it?" verdict per entry — a tri-state read (yes / no /
 * never answered) needs a map rather than `foldCleared`'s set, since absence
 * and "answered no" have to stay distinguishable.
 */
export function foldWorthIt(events: LedgerEvent[]): Map<string, boolean> {
  const verdicts = new Map<string, boolean>();
  for (const e of events) {
    if (e.type === 'worth_it') verdicts.set(e.entryId, e.worthIt);
  }
  return verdicts;
}
